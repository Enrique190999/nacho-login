...
router.post("/", (req, res, next) => {
  try {
    return res
      .status(200)
      .json("/api/ads Crear una oferta de trabajo o curso (admin)");
  } catch (error) {
    return next(error);
  }
});
...
router.put('/',(req,res,next) => {
    try {
        return res
          .status(200)
          .json("/api/ads Editar datos de una oferta de trabajo o curso (admin)");
      } catch (error) {
        return next(error);
      }
})
...
router.delete('/',(req,res,next) => {
    try {
        return res
          .status(200)
          .json("/api/ads Borrar una oferta de trabajo o curso de la base de datos (admin)");
      } catch (error) {
        return next(error);
      }
})
